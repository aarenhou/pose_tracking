    ...
    with camera_lock:
        try:
            if camera is not None:
                return jsonify({'status': 'error', 'message': 'Camera已在運作中'})
            ...
            if not camera.isOpened():
                camera = None
                return jsonify({'status': 'error', 'message': 'Camera無法開啟'})
            ...
            if not ret or test_frame is None:
                camera.release()
                camera = None
                return jsonify({'status': 'error', 'message': 'Camera無法讀取影像'})
            
            return jsonify({'status': 'success', 'message': 'Camera已經啟動'})
            
        except Exception as e:
            if camera is not None:
                camera.release()
                camera = None
            return jsonify({'status': 'error', 'message': f'Camera啟動失敗: {str(e)}'})
...
def stop_camera():
    ...
    with camera_lock:
        try:
            ...
            return jsonify({'status': 'success', 'message': 'Camera已經停止'})
        except Exception as e:
            return jsonify({'status': 'error', 'message': f'Camera無法停止運作: {str(e)}'})
...
def start_recording():
    ...
    try:
        if camera is None:
            return jsonify({'status': 'error', 'message': '請先開啟Camera'})
...
def stop_recording():
    # Stop recording and save the csv file
    global is_recording, recorded_data, start_time
    ...
    try:
        ...
        filename = f'recordings/pose_data_{timestamp}.csv'
...
if __name__ == '__main__':
    app.run(debug=True, threaded=True, host='127.0.0.1', port=10000)
